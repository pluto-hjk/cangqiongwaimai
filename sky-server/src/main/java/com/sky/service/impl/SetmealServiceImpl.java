...
public class SetmealServiceImpl implements SetmealService {
    ...
    public void startOrStop(Integer status, Long id) {
        ...
        if(status.equals(StatusConstant.ENABLE)){  //1  启用
            ...
            if(dishes != null && dishes.size() > 0){//判断套餐中是否包含的有菜品，有才走if判断
                dishes.forEach(dish -> {
        ...
        /**
         * 方法二：查询返回的直接就是状态列表，而不是菜品数据
         *
         *         if(status.equals(StatusConstant.ENABLE)){
         *             List<Integer> statusList = dishMapper.getDishStatusBySetmealId(id);
         *             if (statusList != null && statusList.contains(StatusConstant.DISABLE)) {
         *                 throw new SetmealEnableFailedException(MessageConstant.SETMEAL_ENABLE_FAILED);
         *             }
         *         }
         */
